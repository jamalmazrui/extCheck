...
- **Friendlier source-field parsing.** When you supply a single path, you no longer need to put quotes around it just because the path contains spaces. extCheck tests the entire trimmed source field as a single spec first; only when it is not a usable single spec does it fall back to space-tokenization. Quotes are only required when supplying multiple specs and at least one contains a space. **Bare directory paths (e.g. `C:\Documents`) are now expanded automatically** to all supported file types in that directory — equivalent to `C:\Documents\*.*` — so the trailing wildcard is no longer required.
